...
export default function TaskItem({ task, onToggle, onEdit, onDelete }: TaskItemProps) {
  ...
  return (
    <div className={`bg-white p-4 rounded-lg border border-gray-200 shadow-sm ${task.completed ? 'opacity-75' : ''}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
            ...
            {task.actual_time && (
              <span>実績: {task.actual_time}分</span>
            )}
            {task.completed_at && (
              <span>完了: {new Date(task.completed_at).toLocaleDateString()}</span>
            )}
          </div>
        </div>
